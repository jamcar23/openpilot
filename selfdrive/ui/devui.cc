...
static void dev_ui_draw_desired_steering_angle(UIState *s, Rect *rel_rect, const Rect &rect) {
  ...
  if (true) {
    ...
    if (s->scene.controls_state.getEnabled()) {
      //show Orange if more than 6 degrees
      //show red if  more than 12 degrees
      if(((int)(s->scene.controls_state.getCurvature()) < -6) || ((int)(s->scene.controls_state.getCurvature()) > 6)) {
        val_color = COLOR_YELLOW;
      }
      if(((int)(s->scene.controls_state.getCurvature()) < -12) || ((int)(s->scene.controls_state.getCurvature()) > 12)) {
        val_color = COLOR_RED;
      }
      // steering is in degrees
      snprintf(val_str, sizeof(val_str), "%.0f°",(s->scene.controls_state.getCurvature()));
    } else {
       snprintf(val_str, sizeof(val_str), "-");
    }
      snprintf(uom_str, sizeof(uom_str), "");
